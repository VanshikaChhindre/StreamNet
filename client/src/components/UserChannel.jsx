...
const Channel = () => {
  ...
  const { data : userApi, isSuccess : isUserApiSuccess } = useUserChannelQuery(channel.username)
  const { data : videoApi, isSuccess : isVideoApiSuccess } = useUserVideosQuery(userData?._id, { skip: !userData?._id });
  const { data : tweetApi, isSuccess : isTweetApiSuccess } = useUserTweetsQuery(userData?._id, { skip: !userData?._id });
    ...
    ( <div className='w-full min-h-screen bg-pink-400 flex items-end justify-end'>
        <main className='w-[79vw] min-h-[100vh] bg-background text-text flex flex-col'>
         <section className='w-full h-[25rem] bg-background text-text mt-[4rem] flex flex-col'>
           ...
           style={{backgroundImage: `url(${userData?.coverImage ? userData.coverImage : coverImageDefault})`}}
           ></span>
           <span className='w-full h-[50%]  flex'>
             ...
             <div className='w-[75%] h-full  p-5 flex items-center justify-between'>
               ...
               <h1 className='text-3xl font-semibold'>{userData.fullName}</h1>
